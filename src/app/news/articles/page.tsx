    ...
    <main className="relative min-h-screen bg-[#07070a] text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-28 pb-20">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-3">Breaking News</h1>
          <p className="text-neutral-500 text-sm">Stay ahead of the curve. Breaking stories, industry shifts, and festival coverage.</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <input type="text" placeholder="Search news..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/[0.03] border border-neutral-800/50 rounded-xl pl-11 pr-4 py-3 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-rose-500/30 transition-colors" />
        </div>

        <div className="flex items-center gap-1 mb-6 p-1 bg-white/[0.02] rounded-lg w-fit">
          {([{ key: "newest" as SortKey, label: "Newest" }, { key: "popular" as SortKey, label: "Popular" }, { key: "most-discussed" as SortKey, label: "Most Discussed" }]).map((tab) => (
        ...
        <p className="text-[10px] tracking-[0.2em] uppercase text-neutral-600 mb-6">{filtered.length} article{filtered.length !== 1 ? "s" : ""}</p>
        ...
        {filtered.length === 0 && <div className="text-center py-20"><p className="text-neutral-600 text-sm">No articles found.</p></div>}
      </div>
    </main>
  );
}
