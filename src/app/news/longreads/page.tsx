    ...
    <main className="relative min-h-screen bg-[#07070a] text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-28 pb-20">
        <div className="flex items-center gap-2 mb-10 text-[10px] tracking-[0.2em] uppercase text-neutral-500">
          ...
          <Link href="/news" className="hover:text-rose-400 transition-colors">Flick Feed</Link>
          <span className="opacity-30">/</span>
          <span className="text-rose-400/70">Longreads</span>
        </div>

        <div className="mb-10">
          <p className="text-[10px] tracking-[0.4em] uppercase mb-3" style={{ color: "#fb7185", opacity: 0.6 }}>Flick Feed</p>
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-3">Longreads</h1>
          <p className="text-neutral-500 text-sm">Deep dives, retrospectives, and essays. The stories behind the screen.</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <input type="text" placeholder="Search longreads..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/[0.03] border border-neutral-800/50 rounded-xl pl-11 pr-4 py-3 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-rose-500/30 transition-colors" />
        </div>

        <div className="flex items-center gap-1 mb-6 p-1 bg-white/[0.02] rounded-lg w-fit">
          {([{ key: "newest" as SortKey, label: "Newest" }, { key: "popular" as SortKey, label: "Popular" }, { key: "longest" as SortKey, label: "Longest" }]).map((tab) => (
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          <button onClick={() => setCategory(null)} className={`px-3 py-1 rounded-full text-[10px] tracking-[0.1em] uppercase transition-all duration-300 ${category === null ? "bg-rose-500/15 text-rose-300 border border-rose-500/20" : "text-neutral-500 border border-neutral-800/50 hover:border-neutral-700"}`}>All</button>
          {categories.map((cat) => (
        ...
        <p className="text-[10px] tracking-[0.2em] uppercase text-neutral-600 mb-6">{filtered.length} longread{filtered.length !== 1 ? "s" : ""}</p>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[9px] tracking-[0.1em] uppercase text-neutral-500">{l.readTime} min read</span>
                  </div>
        ...
        {filtered.length === 0 && <div className="text-center py-20"><p className="text-neutral-600 text-sm">No longreads found.</p></div>}
      </div>
    </main>
  );
}
