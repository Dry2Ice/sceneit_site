        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-12">
          {popularArticles.map((a) => (
            <Link key={a.id} href={`/publications/articles/${a.id}`} className="block">
              <div className="bg-white/[0.02] border border-neutral-800/30 rounded-xl p-5 transition-all duration-500 hover:bg-white/[0.04] hover:border-rose-500/15">
                ...
                <div className="flex items-center gap-3 text-[10px] text-neutral-600">
                  <span>{a.author}</span><span className="opacity-30">·</span><span>{formatDate(a.date, lang)}</span><span className="opacity-30">·</span><span>{a.likes} {t.forums.likes}</span>
                </div>
              </div>
            </Link>
          ))}
          ...
          {newestArticles.map((a) => (
            <Link key={a.id} href={`/publications/articles/${a.id}`} className="block">
              <div className="bg-white/[0.02] border border-neutral-800/30 rounded-xl p-5 transition-all duration-500 hover:bg-white/[0.04] hover:border-rose-500/15">
                ...
                <p className="text-xs text-neutral-500 line-clamp-2 mb-3">{a.preview}</p>
                <div className="flex items-center gap-3 text-[10px] text-neutral-600">
                  <span>{a.author}</span><span className="opacity-30">·</span><span>{formatDate(a.date, lang)}</span><span className="opacity-30">·</span><span>{a.comments} {t.forums.comments}</span>
                </div>
              </div>
            </Link>
          ))}
          ...
          {popularReviews.map((r) => (
            <Link key={r.id} href={`/publications/reviews/${r.id}`} className="block">
              <div className="bg-white/[0.02] border border-neutral-800/30 rounded-xl p-5 transition-all duration-500 hover:bg-white/[0.04] hover:border-rose-500/15">
                ...
                <div className="flex items-center gap-3 text-[10px] text-neutral-600">
                  <span>{r.author}</span><span className="opacity-30">·</span><span>{formatDate(r.date, lang)}</span><span className="opacity-30">·</span><span>{r.likes} {t.forums.likes}</span>
                </div>
              </div>
            </Link>
          ))}
          ...
          {newestReviews.map((r) => (
            <Link key={r.id} href={`/publications/reviews/${r.id}`} className="block">
              <div className="bg-white/[0.02] border border-neutral-800/30 rounded-xl p-5 transition-all duration-500 hover:bg-white/[0.04] hover:border-rose-500/15">
                ...
                <p className="text-xs text-neutral-500 line-clamp-2 mb-3">{r.preview}</p>
                <div className="flex items-center gap-3 text-[10px] text-neutral-600">
                  <span>{r.author}</span><span className="opacity-30">·</span><span>{formatDate(r.date, lang)}</span><span className="opacity-30">·</span><span>{r.comments} {t.forums.comments}</span>
                </div>
              </div>
            </Link>
          ))}
          ...
          {popularLongreads.map((l) => (
            <Link key={l.id} href={`/publications/longreads/${l.id}`} className="block">
              <div className="bg-white/[0.02] border border-neutral-800/30 rounded-xl p-5 transition-all duration-500 hover:bg-white/[0.04] hover:border-rose-500/15">
                ...
                <div className="flex items-center gap-3 text-[10px] text-neutral-600">
                  <span>{l.author}</span><span className="opacity-30">·</span><span>{formatDate(l.date, lang)}</span><span className="opacity-30">·</span><span>{l.likes} {t.forums.likes}</span>
                </div>
              </div>
            </Link>
          ))}
          ...
          {newestLongreads.map((l) => (
            <Link key={l.id} href={`/publications/longreads/${l.id}`} className="block">
              <div className="bg-white/[0.02] border border-neutral-800/30 rounded-xl p-5 transition-all duration-500 hover:bg-white/[0.04] hover:border-rose-500/15">
                ...
                <p className="text-xs text-neutral-500 line-clamp-2 mb-3">{l.preview}</p>
                <div className="flex items-center gap-3 text-[10px] text-neutral-600">
                  <span>{l.author}</span><span className="opacity-30">·</span><span>{formatDate(l.date, lang)}</span><span className="opacity-30">·</span><span>{l.comments} {t.forums.comments}</span>
                </div>
              </div>
            </Link>
          ))}
