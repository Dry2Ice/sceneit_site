    ...
    <main className="relative min-h-screen bg-[#07070a] text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-28 pb-20">
        <div className="flex items-center gap-2 mb-10 text-[10px] tracking-[0.2em] uppercase text-neutral-500">
          ...
          <span className="opacity-30">/</span>
          <span className="text-amber-400/70">Brackets</span>
        </div>

        <div className="mb-10">
          <p className="text-[10px] tracking-[0.4em] uppercase mb-3" style={{ color: "#fbbf24", opacity: 0.6 }}>Binge Buddy</p>
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-3">Brackets</h1>
          <p className="text-neutral-500 text-sm">Head-to-head showdowns. Pick the best, round by round, until one champion remains.</p>
        </div>
        ...
        <div className="relative mb-6">
          ...
          <input type="text" placeholder="Search brackets..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/[0.03] border border-neutral-800/50 rounded-xl pl-11 pr-4 py-3 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-amber-500/30 transition-colors" />
        </div>

        <div className="flex items-center gap-1 mb-6 p-1 bg-white/[0.02] rounded-lg w-fit">
          {([{ key: "popular" as SortKey, label: "Popular" }, { key: "newest" as SortKey, label: "Newest" }, { key: "participants" as SortKey, label: "Most Entries" }]).map((tab) => (
        ...
        <p className="text-[10px] tracking-[0.2em] uppercase text-neutral-600 mb-6">{filtered.length} bracket{filtered.length !== 1 ? "s" : ""}</p>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[9px] tracking-[0.1em] uppercase text-neutral-500">{b.participants} entries</span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-neutral-600">
                    ...
                    <span className="opacity-30">·</span>
                    <span>{b.totalPlayed.toLocaleString()} played</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {filtered.length === 0 && <div className="text-center py-20"><p className="text-neutral-600 text-sm">No brackets found.</p></div>}
      </div>
    </main>
  );
}
