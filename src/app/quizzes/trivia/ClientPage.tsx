...
export function TriviaPage({ t }: PageProps) {
    ...
    <main className="relative min-h-screen bg-[#07070a] text-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-28 pb-20">
        ...
        <div className="flex items-center gap-1 mb-6 p-1 bg-white/[0.02] rounded-lg w-fit">
          {([{ key: "popular" as SortKey, label: t.popular }, { key: "newest" as SortKey, label: t.newest }, { key: "hardest" as SortKey, label: t.hardest }]).map((tab) => (
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[9px] tracking-[0.2em] uppercase text-amber-400/50">{trivia.category}</span>
                    <span className="text-[9px] text-neutral-700">·</span>
                    <span className="text-[9px] tracking-[0.1em] uppercase text-neutral-500">{trivia.questionsCount} {t.questions}</span>
                  </div>
                  <h3 className="text-sm sm:text-base font-semibold text-white/90 mb-1.5 group-hover:text-amber-300 transition-colors">{trivia.title}</h3>
                  <p className="text-neutral-500 text-xs sm:text-sm leading-relaxed line-clamp-2 mb-3">{trivia.preview}</p>

                  {/* Score bar */}
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-[10px] text-neutral-500">{t.avgScore}</span>
                    <div className="flex-1 h-1.5 bg-white/[0.04] rounded-full overflow-hidden max-w-[120px]">
                      <div className="h-full rounded-full bg-gradient-to-r from-amber-500 to-amber-400/60" style={{ width: `${trivia.avgScore}%` }} />
                    </div>
                    <span className="text-[10px] text-amber-400/60">{trivia.avgScore}%</span>
                  </div>
