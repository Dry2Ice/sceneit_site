...
export function LoginForm({ t }: LoginFormProps) {
  ...
  return (
    <main className="relative min-h-screen bg-[#07070a] text-white flex items-center justify-center px-6">
      ...
      <div className="absolute w-[500px] h-[500px] rounded-full blur-[200px] opacity-15" style={{ background: "rgba(167, 139, 250, 0.3)", right: "30%", bottom: "20%" }} />
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <h1 className="text-2xl font-bold tracking-tight mb-2">{t.signInLink}</h1>
        <p className="text-neutral-500 text-sm mb-8">{t.welcomeBack}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input name="password" type="password" required className="w-full bg-white/[0.03] border border-neutral-800/50 rounded-lg px-4 py-2.5 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-violet-500/30 transition-colors" placeholder={t.yourPassword} />
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full py-2.5 bg-violet-500/15 hover:bg-violet-500/25 text-violet-300 border border-violet-500/20 rounded-lg text-[10px] tracking-[0.2em] uppercase font-medium transition-all duration-300 disabled:opacity-50">
            {loading ? t.signingIn : t.signInLink}
          </button>
        </form>

        <p className="text-neutral-600 text-xs mt-6 text-center">
          {t.noAccount}{" "}
          <Link href="/register" className="text-violet-400 hover:text-violet-300 transition-colors">{t.createOne}</Link>
        </p>
      </div>
    </main>
  );
